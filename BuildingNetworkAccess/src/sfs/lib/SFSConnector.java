...
public class SFSConnector {
	...
	public JSONArray getChildren(String path){
		try{
			JSONObject resp = new JSONObject(CurlOps.get("http://" + host + ":" + port +path));
			return resp.getJSONArray("children");
		} catch(Exception e){
	...
	public long getTime(){
		try{
			JSONObject resp = new JSONObject(CurlOps.get("http://" + host + ":" + port +"/time"));
	 ...
	 * @return true if successful, false otherwise.
	 */
	public boolean bulkdDataPost(String path, String pubid, JSONArray data){
		JSONObject responseObj=null;
		try{
			JSONObject spec = new JSONObject();
			spec.put("path", path);
			spec.put("pubid", pubid);
			spec.put("data", data);
			responseObj = CurlOps.postAndGetResponse(spec.toString(), sfsurl.toString()+ path + "?bulk=true");
			Log.i("ConnApp::bulkResourceCreate", responseObj.toString());
			if(responseObj.has("response_code") && responseObj.getInt("response_code")==200)
				return 	true;
			return false;
		} catch(Exception e){
			e.printStackTrace();
