<html>
<head>
<script type="text/javascript" src="lib2/jquery-1.6.1.min.js"> </script>
</head>
...
function detach(){
	...
	jQuery.post("lib2/php/detach.php", input_data, registerRet);
}
...
function registerItem(qrc, host, port){
	...
	jQuery.post("lib2/php/register_item.php", input_data, registerRet);
}
...
function clickFunc(){
	...
	data.host=v1;
	data.port=v2;
	data.deployment="DEMO";
	jQuery.post("lib2/php/mobileLib.php", data, mobileRet);
}
...
function isbound($path){
	global $url;
	//$respobj=get("http://".$url["sfs_host"].":".$url["sfs_port"]."/demo/qrc/".$qrc);
